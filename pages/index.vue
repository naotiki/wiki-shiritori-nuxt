...
export default Vue.extend({
  ...
  mounted() {
    if (localStorage.getItem(dbName) != null) {
      let isLoad = confirm("セーブデータがあります。\n読み込む場合は「OK」\n削除して新しく始める場合は「キャンセル」");
      if (isLoad) {
        this.parseSaveData()
      } else {
  ...
  methods: {
    ...
    async shiritoriLogic(input: string) {
      ...
      if (this.nextWord !== firstLetter[0]) {
        this.say({text: `「${this.nextWord}」から言葉を初めてね！`, dir: "left"})
        return
      }
      ...
      const [hiraganaResult, katakanaResult] = await Promise.all([this.wikipediaSearch(lastUserStrings[0]), this.wikipediaSearch(lastUserStrings[1])])
    ...
    async parseString(str: string, isFirst: boolean) {
      ...
      if (/[\u3041-\u3096]/g.test(str.slice(range[0], range[1]))) {
        return [str.slice(range[0], range[1]), hiraToKana(str.slice(range[0], range[1]))]
      } else if (/[\u30A1-\u30F6]/g.test(str.slice(range[0], range[1]))) {
        return [kanaToHira(str.slice(range[0], range[1])), str.slice(range[0], range[1])]
      }
